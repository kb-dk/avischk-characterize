...
<ValidationResultImpl>
  ...
  <testAssertions>
	...
	<s:rule context="/ValidationResultImpl/testAssertions/testAssertions/ruleId">
            <s:assert test="not(clause = '6.1.7')">6.1.7 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.1.11')">6.1.11 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.2.6')">6.2.6 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.3.4')">6.3.4 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.3.5')">6.3.5 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.3.3.1')">6.3.3.1 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.3.6')">6.3.6 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.2.10')">6.2.10 Is not allowed.</s:assert>
            <s:assert test="not(clause = '6.3.2')">6.3.2 Is not allowed.</s:assert>
        </s:rule>
    </s:pattern>
</s:schema>
